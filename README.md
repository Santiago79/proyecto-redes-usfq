    ...
    subgraph "Host Físico"
        subgraph "RED DOCKER: 172.20.0.0/16"
            ...
            subgraph "Subred Privada - 172.20.20.0/24"
                ...
                PHPMYADMIN["📋 phpMyAdmin<br/>IP dinámica (172.20.20.0/24)<br/>Acceso desde host: http://localhost:8081"]
            end
...
4. **`red_monitoreo` (172.20.40.0/24):** Aloja la capa de observabilidad. El `router` también está conectado a esta subred para dar coherencia topológica. *Importante:* El monitoreo funciona mediante exporters multi-interfaz con conectividad directa a las redes objetivo. Esto evita que el scraping de Prometheus sature el plano de datos ruteado durante los ataques.
