...
capture_support_ready() {
  docker exec router sh -lc 'test -d /captures && test -d /opt/lab_scripts/monitor && test -r /opt/lab_scripts/monitor/start_capture.sh && test -r /opt/lab_scripts/monitor/stop_capture.sh && which tcpdump >/dev/null 2>&1'
}
...
check "Capturas Wireshark listas" capture_support_ready
